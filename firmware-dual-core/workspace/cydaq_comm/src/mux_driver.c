...
//	}
	...
	XGpioPs_SetDirection(&GPIO_MUX, MUX_GPIO_CHANNEL, 0x00);
...
u8 muxSetActiveFilter( filters_e filterSelect){
	...
	u8 mask =  ((u8) XGpioPs_Read(&GPIO_MUX, MUX_GPIO_CHANNEL) & (0x0E)) | ((config->muxSelectPins << FLTR_PIN_OFFSET)
				| (config->enablePin << EN_PIN_OFFSET) | (config->hlToggle << HL_TGLE_PIN_OFFSET));
	//writes mask to GPIO channel
	XGpioPs_Write(&GPIO_MUX, MUX_GPIO_CHANNEL, mask);
...
u8 muxSetInputPins( inputs_e inputSelect){
	...
	u8 mask =  ((u8) XGpioPs_Read(&GPIO_MUX, MUX_GPIO_CHANNEL) & (0xF0)) | ((config->muxSelectPins << INPUT_PIN_OFFSET)
