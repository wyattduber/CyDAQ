...
def check_params(data, autocorrect=True):
    ...
    Args:
        autocorrect: enables autocorrect if the user wants it
    ...
    # Instantiate the dict that contains the messages that check_params creates telling the user what went wrong with
    # the configuration There are three types, Error, Warnings, and System Errors are problems encountered that will
    # prevent the configuration of the CyDaq and must be addressed to continue Warnings are problems that should be
    # addressed, but dont have to be. Some errors will become warnings if autocorrect is on System are information
    # for the developer that can give insight into the pipeline of configuration if any problems arise
    ...
    # Because spell checking does not use the dependency tree, we will first check if the parameters are valid,
    # then spell check if any problems arise
    valid = check_valid(data, parameter_dict)
    if not valid:
        check_spelling(data,
                       autocorrect)  # There is a bug here where even though digital in and out dont need corner
        # frequencies, this will still return errors because they are not found in the params. This wont matter in
        # validation because validation follows the dependencies of the paramters.
...
def check_spelling(data, autocorrect):
    ...
    # If the correction is wrong, the user is responsible for using the right spelling of the parameter
    if autocorrect:
        # In the sets returned from check_set, the key is the original word, and the value is the word that
        # autocorrect believes to be the intended word
        ...
        for key in values["misspelled"][0].keys():
            ...
            # Create a message to notify the user of the correction
            data["messages"]["warnings"].append("'{}' corrected to '{}'".format(key, value))
            # Because you cannot directly find the key in a dict based on the value, we will traverse all keys in the
            # dict, and if the value matches the misspelled value, a correction will be applied
        ...
        for key in keys["misspelled"][0].keys():
            valid = False
            value = keys["misspelled"][0][key]
            data["messages"]["errors"].append(
                "'{}' is misspelled, use 'autocorrect=True' to correct to '{}'".format(key, value))
        for key in values["misspelled"][0].keys():
            valid = False
            value = values["misspelled"][0][key]
            data["messages"]["errors"].append(
                "'{}' is misspelled, use 'autocorrect=True' to correct to '{}'".format(key, value))
    # If there is a word within the parameters that cannot be matched to a vocab word, then it is an invalid entry,
    # return false Or there is a word that the checker expects to find, but cannot, then return false
...
def check_word(word, set):
    ...
    min_diff = 999999999
    min_word = ""
    # For every word in the set, find the distace from the word to the vocab and keep track of the closest word and
    # the distace
    for value in set:
...
def check_set(data_p, vocab_p):
    ...
    # Check every remaining word against the given vocabulary, if a match is found then place into 'misspelled' set
...
def check_valid(data, dep_dict):
    ...
    # Extract the parameters from the given dataset and establish the current node pointer to be the root of the
    ...
    while node_pr is not None:
        ...
        if node_type == "Integer":
            # If the layer type is an integer then the given parameter must be between the bounds set by the
            # dependency structure
...
class ParameterConstructor():
    ...
    def inputRec(self, response):
        ...
        if type(response) is int and self.dep_dict[layer]["Type"] == "Integer":
            if self.dep_dict[self.node]["Bounds"][0] <= response <= self.dep_dict[self.node]["Bounds"][1]:
                self.params[layer] = response
